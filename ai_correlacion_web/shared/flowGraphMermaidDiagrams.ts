...
/** Bloco Mermaid `flowchart TB` alinhado ao `flow-graph.json` (sem cercas markdown). */
export function buildContradefFlowchartMermaid(flowGraph: FlowGraph): string {
...
/** Bloco Mermaid `mindmap` (secção 2 do relatório .md). */
export function buildContradefMindmapMermaid(flowGraph: FlowGraph, classification: string): string {
  const nodes = flowGraph.nodes;
  const edges = flowGraph.edges;
  const lines: string[] = [];
  lines.push("mindmap");
  ...
  for (const e of edges) {
    ...
    const list = phaseToApis.get(phaseLabel) ?? [];
    if (!list.includes(tgt.label)) list.push(tgt.label);
    phaseToApis.set(phaseLabel, list);
  }
